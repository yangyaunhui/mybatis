...
//Example 原本英文:例子  意思翻译:给程序员一个增删改查的动态例子,不需要对单表进行增删改查
//2.它的xml中可以作为我们模仿的(动态sql)例子
//经过源码查看,得出的结论:
//Example类是human的一个补充,它把human的属性进行了一个封装
//把human所有字段都进行一个条件语句的封装(not,is null,like,between...and...,or(没它),in,>,<,>=,<=,<>,!=) 封装
//该Example类就是一个单表的自定义查询类---功效:不用在对单表进行手写crud(增删改查)
public class HumanExample {
    protected String orderByClause;//排序  desc   asc

    protected boolean distinct;//去重

    protected List<Criteria> oredCriteria;//查询规范、标准的类

    private Integer limit;//分页

    private Long offset;//分页的
